...
- `http://localhost:3333/admin` - The Dashboard to create and populate data
- `http://localhost:3333/graphql` - GraphQL Playground to test your queries
...
```bash
$ curl -X POST http://localhost:3333/games/populate

# you can pass query parameters like:
$ curl -X POST http://localhost:3333/games/populate?page=2
$ curl -X POST http://localhost:3333/games/populate?search=simcity
$ curl -X POST http://localhost:3333/games/populate?sort=rating&price=free
$ curl -X POST http://localhost:3333/games/populate?availability=coming&sort=popularity
```
...
And you can access `localhost:3333/admin` with the following credentials:
